...
export default function POSPage() {
  ...
  // Order mode
  const [cheDoTaoDon, setCheDoTaoDon] = useState<CheDoTaoDonHang>(
    CheDoTaoDonHang.CHON_DICH_VU_TRUOC,
  );
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        console.log(
          "[POS] Loaded customers for autocomplete:",
          customers.length,
        );
  ...
  useEffect(() => {
    if (!maCuaHang) return;
    const unsubscribe = donHangService.listenByMaCuaHang(
      maCuaHang,
      (orders) => {
  ...
  */
  ...
  const updateStatsFromOrders = (orders: DonHang[]) => {
    ...
    const returnOrders = orders.filter(
      (o) => o.trangThai === TrangThaiDonHang.HOAN_THANH,
    );
    setPendingReturnCount(returnOrders.length);
    setPendingReturnOrders(returnOrders);
  };

  // UI scroll helper for paginated grids on POS machines
  const handleScroll = (e: React.MouseEvent, direction: "left" | "right") => {
    const container = (
      e.currentTarget.closest(".scroll-wrapper") as HTMLElement
    )?.querySelector(".order-scroll-container");
    if (container) {
      container.scrollBy({
        left:
  ...
  const suggestions = useMemo(() => {
    ...
    return allCustomers
      .filter(
        (kh) =>
          removeAccents(kh.hoTen.toLowerCase()).includes(s) ||
          kh.soDienThoai.includes(searchText.trim()),
      )
      .slice(0, 8);
  ...
  const handleQuickCreateCustomer = async () => {
    ...
    if (
      ...
      !/^[0-9]{10,11}$/.test(newCustomerPhone.trim())
    ) {
    ...
    try {
      if (!dv.trangThai) {
        const similar = dichVus.filter(
          (d) =>
            d.maDichVu !== dv.maDichVu &&
            d.trangThai &&
            d.loaiTinhGia === dv.loaiTinhGia,
        );
        const suggestion =
          similar.length > 0
            ? `\nDịch vụ tương tự: ${similar
                .slice(0, 3)
                .map((d) => d.tenDichVu)
                .join(", ")}`
            : "";
          ...
          cart.map((item) =>
            item.maDichVu === dv.maDichVu
              ? {
                  ...
                  trongLuong:
                    dv.loaiTinhGia === LoaiTinhGia.THEO_TRONG_LUONG
                      ? item.trongLuong + 1
                      : item.trongLuong,
                  thanhTien: dichVuService.tinhGia(
                    dv,
                    item.soLuong + 1,
                    dv.loaiTinhGia === LoaiTinhGia.THEO_TRONG_LUONG
                      ? item.trongLuong + 1
          ...
          .map((item) => {
            ...
            const newWeight =
              item.dichVu.loaiTinhGia === LoaiTinhGia.THEO_TRONG_LUONG
                ? Math.max(0, item.trongLuong + delta)
                : item.trongLuong;
            return {
              ...item,
              soLuong: newQty,
              trongLuong: newWeight,
              thanhTien: dichVuService.tinhGia(item.dichVu, newQty, newWeight),
            };
          })
      ...
      setCart(
        cart.map((item) => {
          ...
          return {
            ...item,
            trongLuong: weight,
            thanhTien: dichVuService.tinhGia(item.dichVu, item.soLuong, weight),
          };
        }),
      );
    } catch (err: any) {
  ...
  const handleCreateOrder = async () => {
    ...
    setCreating(true);
    try {
      const services: ChiTietDichVu[] = isMode2
        ? []
        : cart.map(({ dichVu, ...rest }) => rest);
          ...
          const unsubscribe = printService.listenForPrintStatus(
            ...
            (status, errorMsg) => {
              if (status === "PRINTING") {
                ...
                toast.success(`✅ In thành công đơn ${order.maDonHang}`, {
                  id: `print-${jobId}`,
                });
                unsubscribe();
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      toast.success(
        `Thanh toán ${formatCurrency(soTienThanhToan)} thành công!`,
      );
      ...
      const unsubscribe = printService.listenForPrintStatus(
        ...
        (status, errorMsg) => {
          if (status === "PRINTING") {
            ...
            toast.success(`✅ In thành công đơn ${createdOrder.maDonHang}`, {
              id: `print-${jobId}`,
            });
            unsubscribe();
          } else if (status === "FAILED") {
  ...
  const handleMarkDone = async () => {
    ...
    try {
      ...
      if (
        ...
        const services: ChiTietDichVu[] = addServiceCart.map(
          ({ dichVu, ...rest }) => rest,
        );
    ...
    try {
      ...
      if (lookupOrder.tienConLai > 0) {
        ...
        const amount = isBankTransfer
          ? lookupOrder.tienConLai
          : Number(paymentAmount);
        const soTienThanhToan =
          amount > 0
            ? Math.min(amount, lookupOrder.tienConLai)
            : lookupOrder.tienConLai;
  ...
  const addToServiceCart = (dv: DichVu) => {
    const existing = addServiceCart.find(
      (item) => item.maDichVu === dv.maDichVu,
    );
    if (existing) {
      setAddServiceCart(
  ...
  const customerSuggestions = useMemo(() => {
    ...
    return allCustomers
      .filter(
        (kh) =>
          removeAccents(kh.hoTen.toLowerCase()).includes(s) ||
          kh.soDienThoai.includes(customerSearchText.trim()),
      )
      .slice(0, 8);
  ...
  const handleSelectCustomerForLookup = async (kh: KhachHang) => {
    ...
    try {
      ...
      const pending = orders.filter(
        (o) =>
          o.trangThai !== TrangThaiDonHang.DA_GIAO &&
          o.trangThai !== TrangThaiDonHang.DA_HUY,
      );
  ...
  const renderOrderLookup = () => (
    <Card sx={{ mb: 2, overflow: "visible" }}>
      <CardContent
        sx={{ py: 2, "&:last-child": { pb: 2 }, overflow: "visible" }}
      >
        ...
        {customerSearchMode ? (
          ...
          <Box sx={{ position: "relative" }}>
            ...
            {customerPendingOrders.length > 1 && !lookupOrder && (
              <Paper
                variant="outlined"
                sx={{ mt: 1.5, maxHeight: 300, overflow: "auto" }}
              >
                <Typography
                  variant="subtitle2"
                  sx={{ px: 2, pt: 1.5, pb: 0.5, color: "text.secondary" }}
                >
                  📋 Chọn đơn hàng ({customerPendingOrders.length} đơn chưa hoàn
                  thành):
                </Typography>
                <List dense disablePadding>
                        ...
                        primary={
                          ...
                          >
                            ...
                            <Chip
                              ...
                              size="small"
                              sx={{
                                bgcolor: TRANG_THAI_COLORS[order.trangThai],
                                color: "white",
                                fontWeight: 600,
                                fontSize: 11,
                                height: 22,
                        ...
                        secondary={
                          ...
                          >
                            ...
                            >
                              ...
                              {order.tienConLai > 0 &&
                                `(còn ${formatCurrency(order.tienConLai)})`}
                            </Typography>
                          </Box>
                        }
                      />
        ...
        <CardContent>
          ...
          >
            ...
            <Chip
              ...
              sx={{
                bgcolor: TRANG_THAI_COLORS[lookupOrder.trangThai],
                color: "white",
                fontWeight: 600,
              }}
            />
          ...
          {lookupCustomer && (
            <Typography variant="body2" gutterBottom>
              👤 <strong>{lookupCustomer.hoTen}</strong> —{" "}
              {lookupCustomer.soDienThoai}
            </Typography>
          )}
          {lookupOrder.danhSachDichVu.length > 0 ? (
            <Box sx={{ mt: 1 }}>
              ...
              <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                ...
                <Typography fontWeight={700} color="primary">
                  {formatCurrency(lookupOrder.tongTien)}
                </Typography>
              </Box>
              <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                <Typography variant="body2">Đã trả</Typography>
                <Typography variant="body2">
                  {formatCurrency(lookupOrder.tienDaTra)}
                </Typography>
              </Box>
              <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                <Typography variant="body2" fontWeight={600}>
                ...
                <Typography variant="body2" fontWeight={700} color="error">
                  {formatCurrency(lookupOrder.tienConLai)}
                </Typography>
              </Box>
            </Box>
          ) : (
            <Alert severity="warning" variant="outlined" sx={{ mt: 1 }}>
              Chưa có dịch vụ — cần thêm dịch vụ trước khi báo giặt xong
            </Alert>
          )}
        </CardContent>
      </Card>
    ...
    >
          ...
          sx={{
            ...
            ...(posTab === 0
              ...
              : {
                  ...
                  color: "#2e7d32",
                  borderWidth: 2,
                  "&:hover": { bgcolor: "#e8f5e9", borderWidth: 2 },
                }),
          }}
        >
          Tạo đơn
            ...
            sx={{
              ...
              ...(posTab === 1
                ...
                : {
                    ...
                    color: "#e65100",
                    borderWidth: 2,
                    "&:hover": { bgcolor: "#fff3e0", borderWidth: 2 },
                  }),
            }}
          >
            Giặt xong
            ...
            sx={{
              ...
              ...(posTab === 2
                ...
                : {
                    ...
                    color: "#1565c0",
                    borderWidth: 2,
                    "&:hover": { bgcolor: "#e3f2fd", borderWidth: 2 },
                  }),
            }}
          >
            Trả đồ
      ...
      >
          ...
          >
            ...
            >
              ...
              <Chip
                ...
                label={
                  isMode2
                    ? "⏳ Chọn dịch vụ SAU (cân ký sau giặt)"
              ...
              <Card sx={{ mb: 2, overflow: "visible" }}>
                ...
                >
                  ...
                  {customer ? (
                    <Chip
                      label={`${customer.hoTen} - ${customer.soDienThoai}`}
                      color="primary"
                      sx={{ mt: 1 }}
                      onDelete={() => {
                        setCustomer(null);
                      ...
                      <Grid size={{ xs: 6, sm: 4, md: 3 }} key={dv.maDichVu}>
                        <Card
                          ...
                          sx={{
                            ...
                            border: cart.some((c) => c.maDichVu === dv.maDichVu)
                              ? "2px solid"
                              : "1px solid",
                            borderColor: cart.some(
                              (c) => c.maDichVu === dv.maDichVu,
                            )
                              ? "primary.main"
                              : "divider",
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              {dv.loaiTinhGia === LoaiTinhGia.THEO_TRONG_LUONG
                                ? `${formatCurrency(dv.giaTheoKg)}/kg`
                                : formatCurrency(dv.giaTheoSoLuong)}
                            </Typography>
                          </CardContent>
                        </Card>
                      </Grid>
              ...
              {isMode2 && customer && (
                <Alert severity="info" sx={{ mt: 2 }}>
                  Khách hàng đã chọn. Bấm <strong>"Tạo phiếu hẹn"</strong> để in
                  phiếu.
                </Alert>
              )}
            </Box>
            ...
            >
                ...
                {isMode2 ? (
                  <Box sx={{ textAlign: "center", py: 4 }}>
                    <Typography variant="body2" color="text.secondary">
                      Chế độ "Chọn dịch vụ sau"
                    </Typography>
                    <Typography variant="caption" color="text.secondary">
                      Dịch vụ sẽ được xác định sau khi cân ký
                    </Typography>
                  </Box>
                ) : cart.length === 0 ? (
                  <Typography
                    ...
                    >
                      ...
                      >
                        ...
                        <IconButton
                          ...
                          onClick={() =>
                            setCart(
                              cart.filter((c) => c.maDichVu !== item.maDichVu),
                            )
                          }
                        >
                      ...
                      >
                        {item.dichVu.loaiTinhGia ===
                        LoaiTinhGia.THEO_TRONG_LUONG ? (
                          <TextField
                            size="small"
                            type="number"
                            label="Kg"
                          ...
                          >
                            ...
                            >
                              <IconButton
                                size="small"
                                onClick={() =>
                                  updateQuantity(item.maDichVu, -1)
                                }
                              ...
                              <IconButton
                                size="small"
                                onClick={() => updateQuantity(item.maDichVu, 1)}
                                sx={{ width: 36, height: 36 }}
                              >
                                <Add fontSize="small" />
                              </IconButton>
              ...
              <Box sx={{ p: 2 }}>
                ...
                <Button
                  ...
                  disabled={
                    creating || !customer || (!isMode2 && cart.length === 0)
                  }
                  sx={{
                    py: 1.5,
                    minHeight: 52,
                    fontSize: "1rem" /* TC 23: Touch-friendly */,
                    background: isMode2
            ...
            <Box sx={{ flex: 1, overflow: "auto", minHeight: 0 }}>
              ...
              {!lookupOrder && pendingWashOrders.length > 0 && (
                <Card sx={{ mb: 2 }} className="scroll-wrapper">
                  <CardContent
                    sx={{ py: 1.5, "&:last-child": { pb: 1.5 }, px: 0 }}
                  >
                    <Box
                      sx={{
                        display: "flex",
                    ...
                    >
                      {(() => {
                        const pages = [];
                        for (let i = 0; i < pendingWashOrders.length; i += 12) {
                          pages.push(pendingWashOrders.slice(i, i + 12));
                        }
                        return pages.map((page, pageIdx) => (
                          ...
                          >
                            {page.map((order) => {
                              const custName =
                                allCustomers.find(
                                  (c) => c.maKhachHang === order.maKhachHang,
                                )?.hoTen || order.maKhachHang;
                              return (
                                <ListItemButton
                                  key={order.maDonHang}
                                  onClick={() => setLookupOrder(order)}
                                  sx={{
                                    p: 1.5,
                                    borderRadius: 2,
                                    border: "1px solid",
                                  ...
                                  <ListItemText
                                    ...
                                    primary={
                                      ...
                                      >
                                        ...
                                        <Chip
                                          label={
                                            TRANG_THAI_LABELS[order.trangThai]
                                          }
                                          size="small"
                                          sx={{
                                            bgcolor:
                            ...
                            >
                              ...
                              >
                                ...
                                LoaiTinhGia.THEO_TRONG_LUONG ? (
                                  <TextField
                                    ...
                                    onChange={(e) => {
                                      ...
                                      setAddServiceCart(
                                        addServiceCart.map((c) =>
                                          c.maDichVu === item.maDichVu
                                            ? {
                                                ...c,
                                                trongLuong: w,
                                                thanhTien:
                                                  dichVuService.tinhGia(
                                                    c.dichVu,
                                                    c.soLuong,
                                                    w,
                                                  ),
                                ...
                                <IconButton
                                  ...
                                  onClick={() =>
                                    setAddServiceCart(
                                      addServiceCart.filter(
                                        (c) => c.maDichVu !== item.maDichVu,
                                      ),
                                    )
                                  }
                                >
                  ...
                  >
                    ...
                    >
                      {(() => {
                        ...
                        for (
                          let i = 0;
                          i < pendingReturnOrders.length;
                          i += 12
                        ) {
                          pages.push(pendingReturnOrders.slice(i, i + 12));
                        }
                        return pages.map((page, pageIdx) => (
                          <Box
                            key={pageIdx}
                                  ...
                                  <ListItemText
                                    ...
                                    primary={
                                      ...
                                      >
                                        ...
                                        {order.tienConLai > 0 ? (
                                          <Chip
                                            label={`Thiếu ${formatCurrency(order.tienConLai)}`}
                                            size="small"
                                            color="error"
                                            variant="outlined"
                                            sx={{
                        ...
                        >
                          ...
                          <Grid container spacing={0.5} sx={{ mb: 1 }}>
                            {[
                              ...
                              {
                                value: PhuongThucThanhToan.THE_ATM,
                                label: "ATM",
                                icon: <CreditCard />,
                              },
                              {
                                value: PhuongThucThanhToan.VI_DIEN_TU,
                                label: "Ví ĐT",
                                icon: <PhoneAndroid />,
                              },
                            ].map((m) => (
              ...
              >
                {lookupOrder.trangThai === TrangThaiDonHang.DA_GIAO ? (
                  <Alert severity="success">
                    🎉 Đã thanh toán & trả đồ cho khách hàng
                  </Alert>
                ) : lookupOrder.trangThai === TrangThaiDonHang.HOAN_THANH ? (
                  <Button
                    fullWidth
                  ...
                  <Alert severity="info" sx={{ py: 0.5 }}>
                    ...
                    <strong>{TRANG_THAI_LABELS[lookupOrder.trangThai]}</strong>.
                    Cần giặt xong trước khi trả đồ.
                  </Alert>
                )}
              </Box>
            )}
        ...
        >
          ...
          <DialogContent>
            <Typography>
              Bạn có chắc chắn muốn hủy đơn hàng đang tạo? Tất cả dữ liệu sẽ bị
              xóa.
            </Typography>
          </DialogContent>
          <DialogActions>
        ...
        >
          ...
          >
            ...
            <TextField
              ...
              required
              error={
                newCustomerPhone.length > 0 &&
                !/^[0-9]{10,11}$/.test(newCustomerPhone)
              }
              helperText={
                newCustomerPhone.length > 0 &&
                !/^[0-9]{10,11}$/.test(newCustomerPhone)
              ...
              <>
                ...
                >
                  ...
                  <Typography fontWeight={700} color="primary" variant="h6">
                    {formatCurrency(createdOrder.tongTien)}
                  </Typography>
                </Box>
                  ...
                  {[
                    ...
                    <Grid size={6} key={m.value}>
                      <Button
                        fullWidth
                        variant={
                          paymentMethod === m.value ? "contained" : "outlined"
                        }
                        startIcon={m.icon}
                        onClick={() => setPaymentMethod(m.value)}
                        sx={{
                ...
                {Number(paymentAmount) > 0 && (
                  <Box
                    sx={{
                      ...
                      mb: 2,
                      bgcolor:
                        changeAmount > 0 ? "success.50" : "background.default",
                      borderRadius: 2,
                      border: "1px solid",
                      borderColor:
                        changeAmount > 0 ? "success.main" : "divider",
                    }}
                  >
                ...
                <Grid container spacing={1}>
                  {[
                    ...
                    <Grid size={4} key={val}>
                      <Button
                        fullWidth
                        variant={val === "C" ? "outlined" : "contained"}
                        color={
                          val === "C"
                            ? "error"
                            : val === "⌫"
                        ...
                        sx={{
                          ...
                          bgcolor: !["C", "⌫"].includes(val)
                            ? "action.hover"
                            : undefined,
                          color: !["C", "⌫"].includes(val)
                            ? "text.primary"
                            : undefined,
                          "&:hover": {
                            bgcolor: !["C", "⌫"].includes(val)
                  ...
                  {createdOrder.tongTien > 0 && (
                    <Grid size={12}>
                      <Button
                        ...
                        color="primary"
                        onClick={() =>
                          setPaymentAmount(String(createdOrder.tongTien))
                        }
        ...
        >
                  ...
                  >
                    ...
                    >
                      ...
                      <Typography
                        ...
                        color="error"
                      >
                        {formatCurrency(createdOrder.tienConLai)}
                      </Typography>
                    </Box>
                  </Box>
                )}
          ...
          <DialogActions
            sx={{ px: 2, pb: 3, justifyContent: "center", gap: 2 }}
          >
            <Button
              variant="outlined"
              sx={{ minWidth: 120 }}
